...
public class StudentView {
	...
	public void stView(int id) throws SQLException {
		...
		row[0] = "2024-04-19"; // example date
		row[1] = "Present"; // example status

		model.addRow(row); // This adds the row to the model
	...
	public int[] stat(int id) throws SQLException {
		...
		if (x[0] != 0) {
			...
			x[3] = 0; // Set percentage to 0 if total number of classes is zero
		}
		tblupdt(id);
		return x;
	}
